         ...
         {tierData?.map((tier, key) => (
            ...
            { ({ open }) => (
               <>
                  ...
                  <DisclosureButton className={`group flex ${colours[tier.name]} ${hover[tier.name]} ${active[tier.name]} ${tier.name === 'Fuck' ? 'border-2 border-white/35' : ''} rounded-lg w-fit px-4 py-1 duration-200 transition-colors items-center justify-between gap-1`}>
