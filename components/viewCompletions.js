               ...
               <div className='flex flex-col min-h-full min-w-full items-center justify-center'>
                  <DialogPanel transition className='flex flex-col items-center justify-center rounded-2xl px-20 duration-300 ease-out data-[closed]:transform-[scale(95%)] data-[closed]:opacity-0'>        
                     <div className='relative bg-slate-700 rounded-lg'>
                        ...
                        <div className={`${completions.length > 1 ? 'sm:grid grid-cols-2 justify-center gap-4' : 'sm:flex sm:flex-col sm:items-center sm:justify-center'} flex flex-col items-center justify-center`}>
                           {completions.map((level, key) => (
                              <div key={key} className='relative bg-white/10 min-h-40 min-w-72 p-4 m-2 rounded-xl'>
                                 <div className='flex flex-col items-center justify-center'>
                                    <p className='font-semibold text-lg sm:text-xl text-center'><span className='text-xl sm:text-2xl font-inter'>{level.name}</span> by <span className='text-xl sm:text-2xl font-inter'>{level.creators}</span></p>
